"""Dataset-root discovery helpers for LabelTool v7 multi-scale export."""
        ...
        for log_path in sorted(workers_root.glob("*/annotations.jsonl")):
            ...
            for event in latest.values():
                ...
                if relative:
                    ...
                    if key not in store["images"]:
                        store["images"][key] = event
...
def records_from_store(
    ...
    for key, payload in store.get("images", {}).items():
        ...
        if not labels:
            continue
        root_relative = (
            f"{record_dataset}/{source_relative}".strip("/")
            if record_dataset and source_relative
            else relative
        )
        record = {
            "image_id": payload.get("image_id") or path_image_id_from_record(
                {
