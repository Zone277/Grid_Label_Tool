"""Offline multi-annotator project utilities for LabelTool v7."""
...
def path_based_image_id(relative_path: str) -> str:
    """Return a deterministic, human-readable image id derived only from path."""
...
def natural_path_sort_key(path: str | os.PathLike) -> tuple:
    normalized = str(path).replace("\\", "/").casefold()
    key = []
    for part in normalized.split("/"):
        tokens = []
        for token in _NATURAL_TOKEN_RE.findall(part):
            if token.isdigit():
                tokens.append((0, int(token), len(token), token))
            else:
...
def list_images(folder: str | os.PathLike) -> list[Path]:
    ...
    return sorted(
        ...
        key=lambda path: natural_path_sort_key(path.relative_to(root))
    )
