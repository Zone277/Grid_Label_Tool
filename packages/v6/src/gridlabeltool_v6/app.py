...
PERSONAL_DIR = Path(os.environ.get("APPDATA", Path.home())) / "LabelTool" / "v6"
...
DEFAULT_CONFIG = {
    ...
    "aggregation_modes": {
        name: {"enabled": name == "balanced", **thresholds}
        for name, thresholds in MODE_THRESHOLDS.items()
    },
}
DEFAULT_CONFIG["aggregation_modes"][RISK_PRIORITY_MODE] = {"enabled": False}
DEFAULT_PERSONAL = {
    "annotator_id": "",
...
class LabelToolV6:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("LabelTool v6 - 多尺度布料层数标注")
    ...
    def _estimate_export(self, show_dialog=True):
        ...
        for record in records:
            ...
            try:
                with Image.open(path) as image:
                    plan = make_padding_plan(image.width, image.height, base_w, base_h)
                labels = record["labels"]
                if len(labels) != plan.rows or any(len(row) != plan.cols for row in labels):
                    raise ValueError(
                        f"label grid is not {plan.rows}x{plan.cols} "
                        f"for base block {base_w}x{base_h}"
                    )
                for spec in specs:
                    total_windows += expected_window_count(plan.rows, plan.cols, spec)
            except (OSError, ValueError) as exc:
                invalid.append(f"{path.name}: {exc}")
        total_samples = total_windows * len(profiles)
        ...
        text = (
            ...
            f"{len(profiles)} 种模式；预计 {total_samples:,} 个导出文件。"
        )
        if dataset_names:
        ...
        if invalid:
            text += f"\n{len(invalid)} 个图片/尺度组合不可用。"
        self.export_info_label.config(text=text)
        if show_dialog:
            messagebox.showinfo("导出数量", text)
        return total_samples, specs, profiles, records, export_info
    ...
    def _start_export(self) -> None:
        ...
        if estimate is None:
            return
        total, specs, profiles, records, export_info = estimate
        if not records:
        ...
        if not messagebox.askyesno(
            "确认导出",
            f"将从 {len(records)} 张已保存原图导出，预计最多 {total:,} 个模式样本。\n"
            f"同名样本会按当前标签覆盖，是否继续？{source_text}{missing_text}",
        ):
            return
        self.export_cancelled = False
    ...
    def _export_worker_bundle(self) -> None:
        ...
        destination = filedialog.asksaveasfilename(
            ...
            initialfile=f"labeltool_v6_{worker}.zip",
            filetypes=[("ZIP", "*.zip")],
        )
        if not destination:
            return
    ...
    def _export_shortcut_vars(self, variables) -> None:
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile="labeltool_v6_shortcuts.json",
            filetypes=[("JSON", "*.json")],
        )
        if path:
