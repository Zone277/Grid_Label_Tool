...
DEFAULT_CONFIG = {
    "layer_colors": {"单层": "#66cc66", "多层": "#ff6666"},
    "direction_colors": {
        "左上": "#f1c40f",
        "上": "#f39c12",
        "右上": "#e67e22",
...
class EnhancedGridLabelTool:
    ...
    def _save_config(self):
        ...
        self.config_data["transparent_layers"] = sorted(self.transparent_layers)
    ...
    def _build_ui(self):
        ...
        ttk.Label(sec8, text="将导出到 2class/单层、多层 和 8class/八个方向、未标。", wraplength=420, foreground="#555555").pack(fill=tk.X, padx=5, pady=(0, 5))
    ...
    def _ensure_default_colors(self):
        for label in self.layer_options:
            self.layer_colors.setdefault(label, DEFAULT_CONFIG["layer_colors"].get(label, "#cccccc"))
    ...
    def _scan_existing_exports(self):
        ...
        two_class_root = os.path.join(self.output_folder, "2class")
        if os.path.isdir(two_class_root):
            for layer in self.layer_options:
                layer_dir = os.path.join(two_class_root, layer)
        ...
        eight_class_root = os.path.join(self.output_folder, "8class")
        if os.path.isdir(eight_class_root):
            for direction in self.direction_options:
                direction_dir = os.path.join(eight_class_root, direction)
    ...
    def _refresh_canvas(self, recompute_fit=False):
        ...
        if self.original_image is None:
            self.canvas.create_text((self._canvas_w or 500) // 2, (self._canvas_h or 500) // 2, text="请先选择图片文件夹", fill="#888888", font=("微软雅黑", 16))
            return
    ...
    def _on_canvas_click(self, event):
        if self.original_image is None or not self._cell_rects:
            return
        clicked = None
    ...
    def _remove_old_exports_for_cell(self, image_name, out_name):
        ...
        eight_class_root = os.path.join(self.output_folder, "8class")
        for layer in self.layer_options:
            path = os.path.join(two_class_root, layer, out_name)
        ...
        for direction in self.direction_options + ["未标"]:
            path = os.path.join(eight_class_root, direction, out_name)
            if os.path.exists(path):
                try:
                    os.remove(path)
                except OSError:
