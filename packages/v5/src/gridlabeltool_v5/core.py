...
def normalize_shortcut(value: object, fallback: str) -> str:
    ...
    if not shortcut:
        return fallback
    if len(shortcut) == 1:
        return f"<KeyPress-{shortcut}>"
    return shortcut


def select_label_by_digit(digit: str, labels: Iterable[str]) -> str | None:
    label_list = list(labels)
    if digit in label_list:
        return digit
...
def default_cell(layer: str) -> dict[str, str]:
    return {"layer": layer}
...
def replace_invalid_layers(
    ...
    for key, cell in cells.items():
        ...
        result[key] = {"layer": layer if layer in valid_set else fallback}
    return result


def toggle_layer(current_layer: str, selected_layer: str, default_layer: str) -> str:
    if selected_layer == default_layer:
...
def serialize_layer_annotations(
    filename: str,
    rows: int,
    cols: int,
    cells: Mapping[tuple[int, int], Mapping[str, object]],
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            records.append({"row": row, "col": col, "layer": cell["layer"]})
    return {
        "image": filename,
